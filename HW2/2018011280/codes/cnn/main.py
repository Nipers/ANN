	...
	if args.is_train:
		...
		for epoch in range(1, args.num_epochs+1):
			...
			if epoch == args.num_epochs:
				# file = open("testACC.txt", "a")
				# file.write(str(args.drop_rate) + ": " + str(best_test_acc) + "\n")
				# file.close()
				print(best_test_acc)
				print(best)
			file = open("t_loss_D.txt", "a")
			file.write(str(train_loss) + "\n")
			file.close()
			file = open("v_loss_D.txt", "a")
			file.write(str(val_loss) + "\n")
			file.close()
			file = open("t_acc_D.txt", "a")
			file.write(str(train_acc) + "\n")
			file.close()
			file = open("v_acc_D.txt", "a")
